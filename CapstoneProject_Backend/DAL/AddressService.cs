...
{
    public class AddressService : AddressService1
    {
        ...
        public async Task<int> AddAddressAsync(Address address)
        {
            ...
            parameter.Add(new SqlParameter("@Pincode", address.Pincode));
            parameter.Add(new SqlParameter("@UserId", address.UserId));

            var result = await Task.Run(() => _context.Database
            .ExecuteSqlRawAsync(@"exec AddAddress @UserName, @Email, @Phone, @Location, @Country, @State, @Pincode, @UserId", parameter.ToArray()));
            return result;
        }
